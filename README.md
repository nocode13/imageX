...
Команда для нагрузочного тестирования загрузки изображений:

```bash
# 100 уникальных изображений, 5 запросов/сек
./vendor/bin/sail artisan app:stress-test --count=100 --rps=5

# 500 изображений, 10 запросов/сек (симуляция 864k/день)
./vendor/bin/sail artisan app:stress-test --count=500 --rps=10
```
...
Тест генерирует уникальные изображения (без дедупликации), загружает их через API и мониторит обработку очереди до завершения.
